import fs from 'fs';
import { addDots, dateToString, stringToDate, extractNumberInText, cleanSpaces } from './newFormats';
export function validatePassword(pass) {
    if (!pass)
        return `Password is required!`;
    if (typeof pass !== 'string' && typeof pass !== 'number')
        return 'The password must be defined in text or numeric format!';
    if (typeof pass !== 'string')
        pass = pass.toString();
    if (pass.length < 4)
        return 'The password must contain at least 4 characters!';
    if (pass.length > 30)
        return 'The password must not contain more than 30 characters!';
    return null;
}
...
export function validateIdDocument(doc, min, max, required) {
    if ((doc === undefined || doc === null || doc === '') && !required)
        return null;
    if (typeof doc !== 'string' && typeof doc !== 'number')
        return 'The identity document must be defined in text or numeric format!';
    doc = (typeof doc !== 'number') ? Number(doc).toString() : doc;
    if (!(/^[a-z0-9]{1,50}$/i.test(doc.trim())))
        return 'The identity document must be defined as a number only, without additional characters or leading zeros!';
    return null;
}
export function validateFile(file, exts, route, required) {
    let filetype = typeof file;
    if (((filetype !== 'boolean') && !file) && !required)
        return null;
    if (((filetype !== 'boolean') && !file))
        return 'The file is required!';
    if (filetype !== 'string')
        return 'The file params must be defined in text format';
    if (!!exts) {
        let ext = file.trim().replace(/((\w:?)(\\|\/)(.+(\\|\/)))?(.*\.)/g, '').toLowerCase(), valid = (typeof exts === 'string') ? exts.toLowerCase() === ext : exts.some(el => el.toLowerCase() === ext);
        if (!valid)
            return `The file type is incorrect. The accepted types are: ${(typeof exts === 'string') ? exts : exts.join(', ')}!`;
    }
    if (route === true && !fs.existsSync(file))
        return 'File not found!';
    return null;
}
export function validateDoc(doc, type, min, max, required) {
    let typeOf = typeof doc, acceptedTypes = ['string', 'number', 'null', 'undefined'], acceptedTypesDoc = ['V', 'E', 'P'];
    if (!acceptedTypes.some(el => el == typeOf))
        return 'The ID number must be defined in numeric or text format!';
    if (!doc && required)
        return 'You must enter the identity document number!';
    else if (!required)
        return null;
    if (!type)
        return 'You must define the type of document, that is, if the document is Venezuelan (V), foreign (E), or is it a passport (P)!';
    if (!acceptedTypesDoc.some(el => el == type.toUpperCase()))
        return 'The accepted values ​​for the identity document type are "V", "E" and "P"!';
    if (!min)
        min = 1;
    if (min < 1)
        min = 1;
    ...
    if (typeOf == 'string')
        doc = doc.trim();
    if (isNaN(doc) && doc != addDots(doc))
        return `The format of the identity document is incorrect, that is, it must be a number, and only points are accepted to determine units greater than a thousand!`;
    doc = Number(extractNumberInText(doc)).toString();
    if (doc.length < min)
        return `The length of the identity document must not be less than ${min}!`;
    if (doc.length > max)
        return `The length of the identity document must not be greater than ${max}!`;
    return null;
}
function getLastNumRif(RIF) {
    let rifLetter, rifNumber, sumRIF = 0, multi = [3, 2, 7, 6, 5, 4, 3, 2];
    RIF = RIF.toUpperCase();
    rifLetter = RIF.charAt(0);
    rifNumber = RIF.slice(1);
    if (!/^[vpgej]$/i.test(rifLetter) || rifNumber.length !== 8 || !Number(rifNumber))
        return null;
    switch (rifLetter) {
        case "V":
            sumRIF = (1 * 4);
    ...
    for (let i = 0; i < 8; i++)
        sumRIF += (parseInt(rifNumber.charAt(i)) * multi[i]);
    let EntRIF = parseInt((sumRIF / 11).toString()), Residuo = sumRIF - (EntRIF * 11), DigiVal = 11 - Residuo;
    if (DigiVal > 9)
        DigiVal = 0;
    return DigiVal;
}
export function validateRif(rif, obligatory) {
    if (typeof rif !== 'string' && rif !== undefined && rif !== null)
        return 'The rif must be defined in text format!';
    if (!rif && !obligatory)
        return null;
    if (!rif)
        return 'The Rif is required!';
    if (!/[a-z]-?\d{8}-?\d/i.test(rif))
        return 'The rif format must contain the letter that identifies the type and nine numbers, no more (V133455669 or V-13345566-9)!';
    if (!/^[vjpge]$/i.test(rif.charAt(0)))
        return 'Wrong code format letter!';
    rif = rif.replace(/-/, '').toUpperCase();
    if (parseInt(rif.slice(rif.length - 1)) !== getLastNumRif(rif.slice(0, rif.length - 1)))
        return 'Wrong rif!';
    return null;
}