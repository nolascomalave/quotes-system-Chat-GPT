...
export function verifyValuesInObject(req, obj, values) {
...
function verifyFilesInObject(req, values) {
    let errors = new HandlerErrors(), existsErrors = false, toDelete = [];
    values.forEach((el) => {
    ...
    if (toDelete.length > 1)
        deleteRestUploadedFiles(req, toDelete);
    return errors;
}
export default function verifyRequestParams(obj) {
    return function (req, res, next) {
        let errors = new HandlerErrors();
        if (typeof obj !== 'object')
            obj = { any: obj };
        if ('any' in obj && !!obj.any) {
            if (typeof obj.any === 'string')
                obj.any = [obj.any];
